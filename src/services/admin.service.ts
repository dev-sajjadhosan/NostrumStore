...
const updateCategory = async (id: string, payload: any) => {
  try {
    ...
    if (data.error) {
      return {
        data: null,
        error: { message: data.error || "Category not updated!" },
        details: data,
      };
    }
    return { data, error: null };
...
const getUser = async (params?: PgOptionsRs, options?: serviceOptions) => {
  try {
...
const updateUserStatus = async (id: string, payload: any) => {
  try {
    ...
    if (data.error) {
      return {
        data: null,
        error: { message: data.error || "User role not updated!" },
        details: data,
      };
    }
    return { data, error: null };
